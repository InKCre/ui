...
import { validateTokenSource } from "./lib/token-source.js";
...
function buildNestedMap(tokens: TransformedToken[], pathOffset: number = 0): Record<string, any> {
  ...
  for (const token of tokens) {
    ...
    if (
      ...
      const value = token.value;
      // Shadow lengths are numeric in the source export; CSS box-shadow requires units.
      current[lastKey] =
        token.type === "custom-shadow"
          ? Object.fromEntries(
              Object.entries(value).map(([key, part]) => [
                key,
                ["offsetX", "offsetY", "radius", "spread"].includes(key) && typeof part === "number"
                  ? `${part}px`
                  : part,
              ]),
            )
...
const formatUnoPreset = ({ dictionary }: { dictionary: Dictionary }) => {
  ...
  const buildSysColors = () => {
    ...
    for (const token of lightColorTokens) {
      ...
      // Wind3 resolves compound color names through camelCase keys (on-primary -> onPrimary).
      const themeKey = name.replace(/-([a-z])/g, (_, letter: string) => letter.toUpperCase());
      colorMap[category][themeKey] = `var(${varName})`;
    }
  ...
  const buildRadius = () => {
    ...
    for (const token of radiusTokens) {
      ...
      radiusMap[name] = `var(--sys-radius-${name})`;
    }
  ...
  const buildSpaces = () => {
    ...
    for (const token of spaceTokens) {
      const name = token.path[2];
      spaceMap[name] = `var(--sys-space-${name})`;
    }
  ...
  const buildTypography = () => {
    ...
    for (const token of typoTokens) {
      ...
      const property = toKebabCase(prop);
      typoMap[category][size][property] = `var(--sys-font-${category}-${size}-${property})`;
      typoMap[category][size]["font-family"] = "var(--sys-typo-family-sans)";
      typoMap[category][size]["text-decoration"] = "none";
    }
...
async function main() {
  ...
  try {
    const source: unknown = JSON.parse(readFileSync(tokenPath, "utf8"));
