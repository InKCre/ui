...
ref.space 的 xs/sm/md/lg/xl 分别为 0.25/0.5/1/2/3.5rem，随根字号调整，避免嵌套字体导致间距重复缩放。16px 根字号下保持原有尺寸。圆角、边框、断点与 ref.size 不随此次 spacing 调整改动。Figma 提议须沿用已有 dimension 单位，不能把设计工具显示的像素结果直接覆盖相对单位；单位变更需要在仓库显式迁移。
