...
- **Current Truth**: Execution 01 is implemented and locally verified. Node `22.22.3`, pnpm `11.17.0`, one root lockfile, an explicit dependency build-script allowlist, canonical root commands, generated-output freshness checks, hard declaration diagnostics, and a read-only PR workflow now form the engineering baseline. Both the working tree and a disposable fresh copy pass frozen installation plus the complete root `check`: formatter scope, blocking lint errors, generators, root/Vue type checks, 103 unit tests, package/locales build, and 21 Histoire stories with 108 variants. `InkDialog` tests now assert public behavior; `InkImage` uncontrolled expansion and Scrim close-button behavior are fixed and covered by a patch changeset. Sir accepted the canonical identity `InKCre/ui` / `@inkcre/ui` / `packages/web` / `@inkcre/ui-web`. The existing package boundary remains invalid until Execution 03: the mixed `exports` map still makes Node reject the packed artifact, and utilities/locales declarations remain incomplete. Execution 02 is now authorized because the checked-in npm credential placeholder is still ignored by pnpm as untrusted configuration. GitHub package visibility and the old-package compatibility posture remain unconfirmed. Active posture is Solidify; Sir authorized the bounded Execution 01 commit and explicitly started Execution 02. No push, remote CI run, governance mutation, publish, consumer edit, or remote rename has been authorized.
- **Next Step**: preserve Execution 01 as a bounded commit, then implement the authorized registry-authentication boundary in Execution 02. Before Execution 05, confirm whether `@inkcre/ui-web` remains private and whether to accept the recommended freeze/deprecate/no-wrapper policy for `@inkcre/web-design`.
...
- Active posture: Solidify. Execution 01 is locally verified and authorized for commit; Execution 02 is authorized to start.
...
- `.npmrc` stores an environment-expanded registry credential in repository-controlled configuration; pnpm reports that it ignores this credential source.
